...
function Ajustar() {
  ...
  const idMal = params[params.length-1]
  

  useEffect (()=>{
    async function fetchData() {
      try{
        const req = await fetch(`http://localhost:5000/adote/${idMal}`)
        const Mal = await req.json()
        console.log(Mal);
        setNome(Mal.nome)
        setTipo(Mal.tipo)
        setRaca(Mal.raca)
        setVacinado(Mal.vacinado)
        setFotoUrl(Mal.setFotoUrl == "" ? "":Mal.setFotoUrl)
      }
      catch(error){
